...
- **Gate audits only the initial/empty DOM** (`tests/e2e/accessibility.spec.ts:56-71`): the populated listbox + `aria-activedescendant` active state — the very behavior this story changed in `UserSearchInput.tsx` — is never exercised by the audit (no query is typed to render options). Spec explicitly scoped component/dynamic states out of 6.4 (AC #1 "page/route" wording). Strong follow-up for Story 6.1; add a "type a query → options visible → audit" path.
- **Empty-state-only coverage** (`tests/e2e/accessibility.spec.ts:56-71`): `/send` is audited unfunded and `/history` with no transactions, so the funded-balance markup and populated transaction-row markup are never scanned. Data-dependent; needs a seeded account. Pairs with the batch-admin seed work (Epic 5).
- **Combobox ARIA incomplete** (`src/app/(protected)/send/UserSearchInput.tsx:109-121`): the search input exposes `aria-controls`/`aria-activedescendant`/`aria-autocomplete="list"` but no `aria-expanded` and no `role="combobox"`, so a screen reader isn't told when results expand/collapse. Axe-clean (no `role="combobox"` to trigger the rule) and not regressed by this diff — squarely Story 6.1's full-ARIA pass.
- **Component hardcodes global ids** (`src/app/(protected)/send/UserSearchInput.tsx:95,110,144`): `user-search`, `user-search-listbox`, and `user-option-N` are string constants rather than `useId()`-derived. Latent duplicate-id collision (and broken `aria-activedescendant`) if two `UserSearchInput` instances ever co-mount on a single route. Pre-existing; non-manifesting today because /send and /request each mount exactly one. Switch to `useId()` if the component is ever reused within one page.
- **Test-infra: unmigrated DB → false timeout** (`playwright.config.ts:19-22`): `reuseExistingServer: !CI` plus a local DB missing the latest migrations makes `register()` hit a 500, which cascades into every authenticated audit timing out (this actually happened per the story Debug Log; resolved manually via `npm run db:migrate`). Relates to the known credential-less-`.env` / drizzle-kit migrate gotcha. Add a globalSetup migration/health guard so the failure is diagnostic instead of a wall of timeouts.
- **No account teardown** (`tests/e2e/accessibility.spec.ts:56`): each run registers a fresh `e2e_a11y_*` user with no cleanup, so the test DB accumulates accounts indefinitely. Suite-wide convention (every existing e2e spec does this); address in a suite-level cleanup/globalTeardown pass, not piecemeal.
