...
import type { SendMoneyInput } from '@/lib/schemas'
...
// Postgres `integer` (int4) max — balance_cents/amount_cents cannot exceed this.
...
export default function SendPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (res.status === 409 && data.code === 'INSUFFICIENT_BALANCE') {
        const currentBalance = useBalanceStore.getState().balanceCents
        setSubmitError(
          `Insufficient balance — you have ${formatCents(currentBalance)}, this transfer is for ${formatCents(amountCents)}`,
        )
      } else {
